...
from clawinboxrag.parity_harness import BaselineGoldenResult, GoldenQuery, run_golden_parity
...
class Phase2ParityHarnessTests(unittest.TestCase):
    def test_phase2_golden_queries_parity(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            # Baseline captured from the current gmail-rag flow for this deterministic fixture.
            baseline_results = {
            ...
            self.assertEqual(6, report.passed)
            self.assertEqual(0, report.failed)
            self.assertAlmostEqual(1.0, report.parity_score)

            by_class = {row.query_class: row for row in report.by_class}
            self.assertEqual(1, by_class["label+from"].passed)
            self.assertEqual(1, by_class["date_after"].passed)
            self.assertEqual(1, by_class["date_before"].passed)
            self.assertEqual(1, by_class["to_filter"].passed)
            self.assertEqual(1, by_class["direction_received"].passed)
            self.assertEqual(1, by_class["direction_sent"].passed)

            for row in report.results:
                self.assertTrue(row.citations_ok, row)
                self.assertEqual([], row.notes, row)
